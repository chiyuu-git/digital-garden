---
{"dg-publish":true,"permalink":"/programming/front-end/primitive/es/es-proto/"}
---
...
可以看到原型指向完之后，才执行了 Person 函数，此时通过字面量的形式重写了原型，导致实例无法访问
...
上面代码定义了一个 `MyArray` 类，继承了 `Array` 构造函数，因此就可以从 `MyArray` 生成数组的实例。这意味着，**ES6 可以自定义原生数据结构（比如 `Array`、`String` 等）的子类**，这是 ES5 无法做到的。上面这个例子也说明，`extends` 关键字不仅可以用来继承类，还可以用来继承原生的构造函数。因此可以在原生数据结构的基础上，定义自己的数据结构。
...
![es-proto](es-proto.md#原型链的面试题)

## 手动实现一个 Instanceof

![es-proto](es-proto.md#Instanceof%20的底层实现原理)
